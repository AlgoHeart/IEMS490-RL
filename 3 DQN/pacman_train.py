...
def preprocess(obs):
    ...
    return np.reshape(img.astype(np.float).ravel(), [88,80])


def Q_net(image):
    """forward pass of Q-network"""
    image = tf.reshape(image, shape=[-1, 88, 80, 4])
    # hidden layers
    ...
    h_conv2_flat_Q = tf.reshape(h_conv2_Q,[-1, 2304])
...
def Target_net(image):
    ...
    h_conv2_flat_T = tf.reshape(h_conv2_T,[-1, 2304])
...
image_input = tf.placeholder(tf.float32, shape=(None, 4, 88, 80))
...
W_fc1_Q = tf.Variable(tf.truncated_normal([2304, 256], stddev = 0.01))
b_fc1_Q = tf.Variable(tf.constant(0.01, shape = [256]))
W_fc1_T = tf.Variable(tf.truncated_normal([2304, 256], stddev = 0.01))
